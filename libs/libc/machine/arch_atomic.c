...
#define STORE(n, type)                                             \
                                                                   \
  void weak_function nx_atomic_store_##n (FAR volatile void *ptr,  \
...
#define LOAD(n, type)                                                 \
                                                                      \
  type weak_function nx_atomic_load_##n (FAR const volatile void *ptr,\
                                        int memorder)                 \
  {                                                                   \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                    \
                                                                      \
...
#define EXCHANGE(n, type)                                             \
                                                                      \
  type weak_function nx_atomic_exchange_##n (FAR volatile void *ptr,  \
                                            type value, int memorder) \
  {                                                                   \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                    \
...
#define CMP_EXCHANGE(n, type)                                                 \
                                                                              \
  bool weak_function nx_atomic_compare_exchange_##n (FAR volatile void *mem,  \
                                                    FAR void *expect,         \
...
#define FLAG_TEST_AND_SET(n, type)                                           \
                                                                             \
  type weak_function nx_atomic_flags_test_and_set##n (FAR volatile void *ptr,\
                                                     int memorder)           \
  {                                                                          \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                           \
    FAR type *tmp = (FAR type *)ptr;                                         \
...
#define FETCH_ADD(n, type)                                             \
                                                                       \
  type weak_function nx_atomic_fetch_add_##n (FAR volatile void *ptr,  \
                                             type value, int memorder) \
  {                                                                    \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                     \
    FAR type *tmp = (FAR type *)ptr;                                   \
...
#define FETCH_SUB(n, type)                                             \
                                                                       \
  type weak_function nx_atomic_fetch_sub_##n (FAR volatile void *ptr,  \
                                             type value, int memorder) \
  {                                                                    \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                     \
    FAR type *tmp = (FAR type *)ptr;                                   \
...
#define FETCH_AND(n, type)                                             \
                                                                       \
  type weak_function nx_atomic_fetch_and_##n (FAR volatile void *ptr,  \
                                             type value, int memorder) \
  {                                                                    \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                     \
    FAR type *tmp = (FAR type *)ptr;                                   \
...
#define FETCH_OR(n, type)                                             \
                                                                      \
  type weak_function nx_atomic_fetch_or_##n (FAR volatile void *ptr,  \
                                            type value, int memorder) \
  {                                                                   \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                    \
    FAR type *tmp = (FAR type *)ptr;                                  \
...
#define FETCH_XOR(n, type)                                             \
                                                                       \
  type weak_function nx_atomic_fetch_xor_##n (FAR volatile void *ptr,  \
                                             type value, int memorder) \
  {                                                                    \
    irqstate_t irqstate = spin_lock_irqsave(NULL);                     \
    FAR type *tmp = (FAR type *)ptr;                                   \
